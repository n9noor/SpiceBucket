...
class VariantController extends Controller
{
    public function variant_type(Request $request) {
		if($request->session()->get('admin-logged-in') == true){
			$result = ProductVerient::select('product_variants.*')->get();
		} else {
    ...
    public function variant_value(Request $request) {
		if($request->session()->get('admin-logged-in') == true){
			$result = ProductVerientValue::join('product_variants', 'product_variants.id', '=', 'product_variant_values.variant_id')->select('product_variant_values.*', 'product_variants.name AS variant_type_name')->get();
		} else {
    ...
    public function save_variant_type(Request $request){
        ...
        $result = ProductVerient::where('name', $request->varianttype)->first();
        if($request->id != 0){
            $result = ProductVerient::where('name', $request->varianttype)->where('id', '<>', $request->id)->first();
        }
