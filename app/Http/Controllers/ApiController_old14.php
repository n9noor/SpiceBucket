...
class ApiController extends Controller
{
        ...
        if(is_numeric($data->email)){
            ...
            if(!is_null($result) && Hash::check($data->otp, $result->otp)) {
                ...
                return response()->json([
                    'status' => true,
                    'message' => 'User logged in successfully',
                    'data' => ['id' => $result->id, 'emailid' => $result->emailid, 'name' => $result->name, 'phone' => $result->phone, 'dob' => $result->dob, 'tokenId' => $tokenid, 'deviceId' => $data->deviceId, 'firebaseTokenId' => $data->firebaseToken, "profileImage" => $result->image]
                ], 200);
            } else {
                return response()->json([
                    'status' => false,
    ...
    */    
    ...
    public function get_user_details(Request $request) {
        ...
        if(!is_null($result)) {
            ...
            return response()->json([
                'status' => true,
                'message' => 'User found.',
                'data' => ['customer_id'=> $result->id ,'emailid' => $result->emailid, 'name' => $result->name, 'phone' => $result->phone, 'dob' => $result->dob, 'tokenId' => $result->token_id, 'deviceId' => $data->deviceId, 'firebaseTokenId' => $data->firebaseToken, "profileImage" => $result->image]
            ], 200);
        } else {
            return response()->json([
                'status' => false,
    ...
    public function update_customer(Request $request) {
        ...
        $customer->firebase_token_id = $request->input('firebaseToken');
