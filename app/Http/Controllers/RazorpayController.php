...
class RazorpayController extends Controller
{
    ...
    {
         $this->sendMailForOrderTrait( 969 ,'niraj4u2@gmail.com');
        die;
    }
...
/********************** wallet *********************/
 public function paymentwalletcodtest(Request $request)
    {
        ...
        //$payment = $api->payment->fetch($input['razorpay_payment_id']);
			...
			/*$order = $api->order->create([
					'wallet_amount' => $request->Wallet_amount, // amount in paise
					'userid' => $request->userid,
					'currency' => 'INR',
					'payment_capture' => 1, // Auto capture
					'receipt' => 'order_rcptid_'.rand(0,5)
					//'notes' => ['key' => 'value'],
		]);
		$order_id = $order->id;*/
		//echo "<pre>";
		...
		$data = array ( 
				'userid' => $request->userid,
            	'wallet_amount' => (int)($request->Wallet_amount * 100), // Amount should be in paise
				'currency' => 'INR',
				'payment_capture' => 1,		
				'transaction_detail' => 'Test transaction'
		);
		
		//$order = $api->order->create(array('amount' => (int)($request->Wallet_amount * 100), 'currency' => 'INR', 'userid' => $request->userid));
		$razorpayOrder = $api->order->create($data);
		 try {
			$Resultadd =  DB::table('tbl_wallet_cart')->insert($data);	
			$Message = "Data has been added successfully"; 
            return view('walletpayment', compact('Message','razorpayOrder'));
        } catch (\Exception $e) {
		...
		//compact('orderId', 'amount', 'currency')
      // return view('walletpayment', compact('Message', 'data','orderId'));
    }
