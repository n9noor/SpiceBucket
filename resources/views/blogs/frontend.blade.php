						...
						<article class="col-xl-6 col-lg-6 col-md-6 col-sm-4 text-center hover-up mb-30 animated">
							<div class="post-thumb">
								<a href="/blog/{{$blog->category_slug}}/{{$blog->slug}}">
									<img class="border-radius-15" src="{{ !is_null($blog->featured_image) ? (env('APP_ENV') == 'production' ? url('/public/images/blogs/' . $blog->featured_image) : url('/images/blogs/' . $blog->featured_image)) : url('/assets/imgs/no-image-placeholder.png') }}" alt="" />
								</a>
								<div class="entry-meta">
									<!--<a class="entry-meta meta-2" href="/blog/{{$blog->category_slug}}/{{$blog->slug}}"><i class="fi-rs-heart"></i></a>-->
								</div>
							</div>
							<div class="entry-content-2">
								<h6 class="mb-10 font-sm"><a class="entry-meta text-muted" href="/blog/{{$blog->category_slug}}">{{$blog->categoryname}}</a></h6>
								<h4 class="post-title mb-15">
									<a href="/blog/{{$blog->category_slug}}/{{$blog->slug}}">{{$blog->title}}</a>
								</h4>
								<div class="entry-meta font-xs color-grey mt-10 pb-10">
									<div>
										<span class="post-on mr-10">{{date('d F Y', strtotime($blog->created_at))}}</span>
