        ...
        <div class="table-responsive" style="">
            <table class="table table-bordered table-striped" style="">
                ...
                <tbody>
                    ...
                    <tr>
                        <td nowrap><a href="/sellers/orders/view/{{$order->idoforder}}/{{$order->vendor_id}}">View</a></td>
                        ...
                        @endphp
                        <td nowrap><i class="fa fa-rupee-sign"></i> {{number_format($order->totalOrderPrice, 2)}} </td>
                        <td nowrap><i class="fa fa-rupee-sign"></i> {{number_format(($order->OrderGSTPrice + $gstshippingprice), 2)}} </td>
                        @if($order->discount > 0)
                        ...
                        <td style="text-align: center;" nowrap><i class="fa fa-rupee-sign"></i> {{number_format($order->totalOrderPrice + $order->OrderGSTPrice + $order->shipping_charges + $order->cod_charges - $order->discount, 2)}} </td>
                        <td nowrap>{{strtoupper($order->payment_source)}}</td>
                        <td nowrap>{{ucwords($order->payment_status)}}</td>
                        <td nowrap>{{ucwords(($order->order_status == 'cancel') ? "cancelled" : $order->order_status)}}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
