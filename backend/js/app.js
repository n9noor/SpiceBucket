...
$(document).ready(() => {
  ...
  // verify vendor
  $(".verify-vendor-btn").click(function(){
    $.get("/administrator/verify-vendor", {gst_number: $(this).attr("data-gstnumber")}, function(result){
  ...
  $(document).on('change', 'input[type="checkbox"][data-toggle="toggle"]', function() {
    ...
    $.post("/administrator/update-status", {"_token": $('#defaultcsrftoken').val(), column: column, type: type, id: id, status: status}, function(result){
      if(result.status == true){
        toastr["success"]("", (status == true ? "A" : "Ina") + "ctivated successfully.");
      } else {
        toastr["error"]("", "Unable to perform the operation.");
      }
    }, 'json');
  });
